  ...
  return (
    <div className="rounded-2xl border border-[#1E293B] bg-white overflow-hidden shadow-[8px_8px_0px_0px_#E2E8F0]">
      <div className="border-b border-[#1E293B] px-6 py-5">
        <h2 className="text-base font-bold flex items-center gap-2">📸 Posisikan Wajahmu</h2>
      ...
      <div className="flex flex-col items-center p-10">
        <div className="relative mb-8 flex aspect-[4/3] w-full max-w-md items-center justify-center overflow-hidden bg-[#1E293B]">
          ...
          {cameraStarted ? (
            <>
              <div
                className={`pointer-events-none absolute top-1/2 left-1/2 h-[280px] w-[220px] -translate-x-1/2 -translate-y-1/2 rounded-[50%] border-3 ${
                  faceDetected
        ...
        <div className="flex flex-col items-center gap-3">
          {!cameraStarted ? (
            <button
              ...
              className="flex items-center gap-2 rounded-full border-2 border-[#1E293B] bg-[#8B5CF6] px-6 py-3 text-sm font-bold text-white shadow-[3px_3px_0px_0px_#1E293B] transition-all hover:-translate-y-0.5 active:translate-y-0.5 active:shadow-[1px_1px_0px_0px_#1E293B]"
            >
              <FiCamera size={18} />
              Aktifkan Kamera
            </button>
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full border-2 border-[#1E293B] bg-pink-400 px-8 py-3 text-sm font-bold text-white shadow-[3px_3px_0px_0px_#1E293B] transition-all hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <FiAperture size={18} />
              Capture
            </button>
          )}
          <p className="mt-1 text-xs font-medium text-[#64748B]">{autoCaptureMessage}</p>
        </div>
      </div>
    </div>
  );
